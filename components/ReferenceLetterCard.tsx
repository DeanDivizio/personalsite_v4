    ...
    return (
        <Link href={link} className="mb-4">
            <Card className="grid grid-cols-4 p-6 hover:scale-[102%] transition-all duration-300 bg-gradient-to-br from-slate to-slate-dark border-black rounded-lg">
                ...
                {isAlex ? <p className="text-offWhite">{`Co-Founder of `}<span className={ovrtoneFont.className} style={{color: 'var(--green)'}}>OVRTONE</span>{` Media Group`}</p>: <p className="text-offWhite">{subtitle}</p> }
                </div>
            </Card>
        </Link>
    )
